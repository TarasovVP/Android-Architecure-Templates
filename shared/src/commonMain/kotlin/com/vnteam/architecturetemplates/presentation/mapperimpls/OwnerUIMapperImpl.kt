...
class OwnerUIMapperImpl : OwnerUIMapper {
    ...
    override fun mapToImplModelList(fromList: List<Owner>): List<OwnerUI> = fromList.map { mapToImplModel(it) }

    override fun mapFromImplModelList(toList: List<OwnerUI>): List<Owner> = toList.map { mapFromImplModel(it) }
}
